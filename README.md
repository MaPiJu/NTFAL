...
- `[strategy]` — tune the Elder scanner thresholds and ranking weights without editing
  ...
  scale, and score weights. Optional keys may be added for value-zone proximity
  (`value_zone_max_distance_pct`), SafeZone noise (`safezone_lookback_days`,
  `safezone_factor`), and theoretical stop-order expiry (`entry_order_expire_days`);
  if omitted, the built-in defaults are used so existing local `config.toml` files
